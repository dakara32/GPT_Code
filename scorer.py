...
class Scorer:
    ...
    def aggregate_scores(self, ib: Any, cfg):
        ...
        # --- Dividend handling: penalize non-payers ---
        df['HAS_DIV'] = (df['DIV_TTM_PS'] > 0).astype(int)
        y = df['DIV_TTM_PS'].where(df['HAS_DIV'] == 1, np.nan)
        y_non = y.dropna()
        z_yld = pd.Series(robust_z(y_non), index=y_non.index).reindex(df.index)
        penalty = (np.nanmin(z_yld) - 1.0) if len(z_yld.dropna()) else -1.0
        z_yld = z_yld.fillna(penalty)
        ...
        df_z.rename(columns={'MOM_F':'MOM','QUALITY_F':'QAL','YIELD_F':'YLD'}, inplace=True)

        # --- EPS-only penalty: punish negative EPS (GAAP or nEPS) ---
        eps_any = (df['EPS'] > 0) | (df.get('nEPS_ttm', 0) > 0)
        ...
        X = df_z[g_cols].copy()
        X["GRW"] = df_z["GRW"]
        g_score_all = X.mul(pd.Series(cfg.weights.g)).sum(axis=1)
        # print("[GRW apply] PLTR raw/adj:", df_z.loc["PLTR", ["GRW_raw","GRW"]].to_dict())
        # print("[X GRW] PLTR:", float(X.loc["PLTR","GRW"]))
